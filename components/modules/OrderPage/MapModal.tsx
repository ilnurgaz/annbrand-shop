...
import { IAnnbrandAddressData } from '@/types/order'
...
const MapModal = () => {
  ...
  const handleLoadMap = async (initialContainer = pickUpMapRef) => {
    ...
    const setMarkersByLocationsData = (data: IAnnbrandAddressData[]) => {
      data.forEach((item) => {
    ...
    if (!!chosenPickupAddressData.address_line1) {
      const chosenItem = annbrandDataByCity.filter(
    ...
    if (annbrandDataByCity.length) {
      setMarkersByLocationsData(annbrandDataByCity)
    }
